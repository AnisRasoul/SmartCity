...
const defaultHeaders = [
  { label: 'Client', minWidth: '220px' },
  { label: 'Reservation', minWidth: '150px' },
...
const handleStatusChange = (reservationId: string, newStatus: string) => {
  emit('updateStatus', { reservationId, newStatus })
}
</script>
      ...
      <table class="w-full table-auto">
        ...
        <tbody>
          <tr v-for="(item, index) in reservations" :key="index">
            ...
            <td class="py-5 px-4">
              <p class="text-black dark:text-white">{{ item.reservationId }}</p>
            </td>
            <td class="py-5 px-4">
            ...
            <td class="py-5 px-4">
              <select
                class="rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium outline-none"
                :class="{
                  'bg-success text-success': item.status === 'Confirmed',
                  'bg-warning text-warning': item.status === 'Pending',
                  'bg-danger text-danger': item.status === 'Cancelled'
                }"
                :value="item.status"
                @change="handleStatusChange(item.reservationId, ($event.target as HTMLSelectElement).value)"
              >
